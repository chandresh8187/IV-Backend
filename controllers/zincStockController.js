...
const getZincMovements = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(`SELECT m.id, m.movement_type, m.amount_kg,
...
const saveZincMovement = async (req, res) => {
  ...
  try {
    ...
    await connection.query(`UPDATE zinc_stock SET initialized = 1, plant_kg = ?, kettle_kg = ?,
      kg_per_mm = ?, revision = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1`,
    [next.plant_kg, next.kettle_kg, next.kg_per_mm, next.revision]);
    await connection.query(`INSERT INTO zinc_stock_movements
      (request_id, request_hash, movement_type, amount_kg, plant_after_kg, kettle_after_kg, note, actor_user_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [movement.request_id, hash, movement.action, (movement.amountGrams || 0) / 1000, next.plant_kg, next.kettle_kg, movement.note, req.user.id]);
    await connection.commit();
    transaction = false;
    ...
    const message = movement.action === 'transfer'
      ...
      : movement.action === 'adjust'
        ...
        : 'Zinc stock saved.';
    return res.json({ success: true, message, data: serializeStock(next) });
  } catch (error) {
...
module.exports = { getZincStock, getZincMovements, downloadZincMovementsPdf, saveZincMovement };
